...
def query_api(
    question: str, max_results: int = 5, include_sources: bool = True
) -> dict:
    payload = {
        "question": question,
    ...
    try:
        response = requests.post(
            CHAT_API_URL, json=payload, headers=headers, timeout=60
        )
        response.raise_for_status()
        return response.json()
    except requests.exceptions.ConnectionError:
        raise Exception(
            f"Cannot connect to API. Is the backend running at {CHAT_API_URL}?"
        )
...
with ingestTab:
        ...
        if st.button("Upload and Ingest", type="primary", use_container_width=True):
            with st.spinner("Uploading and processing..."):
                try:
                    ...
                    response = requests.post(
                        UPLOAD_API_URL, files=files, headers=headers, timeout=600
                    )
    ...
    try:
        response = requests.get(LIST_API_URL, headers=headers, timeout=5)
        if response.status_code == 200:
            data = response.json()
            col1, col2 = st.columns(2)
            with col1:
